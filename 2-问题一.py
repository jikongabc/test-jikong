...
# 导入 Excel 数据
land_data = pd.read_excel(land_filename, sheet_name=0)
...
for land_type in land_types:
    names = land_data.loc[land_data['地块类型'] == land_type, '地块名称']
    areas = land_data.loc[land_data['地块类型'] == land_type, '地块面积']

    plot_names.extend(names)
    plot_areas.extend(areas)
    plot_types.extend([land_type] * len(names))
...
planting_data_2023 = pd.read_excel(planting_filename, sheet_name=0)

# 添加地块信息（地块类型、地块面积）#
planting_data_2023 = planting_data_2023.merge(plot_info, on='种植地块', how='left')
...
crop_data = pd.read_excel(crop_filename, sheet_name=0)
...
# 导入 Excel 数据
crop_land_data = pd.read_excel(filename, sheet_name=0)

# 检查导入的数据
print('导入的作物-土地适用数据:')
print(crop_land_data.head(30))
...
for i in range(len(crop_land_data)):
    ...
    for token in tokens:
        land_type, season = token

        # 如果包含多个季节，继续分割
        seasons = season.split(' ')
        for season_part in seasons:
            # 添加作物编号、名称、类型到拆解列表中
            crop_ids_all.append(crop_ids[i])
            crop_names_all.append(crop_names[i])
            crop_types_all.append(crop_types[i])

            # 存储地块类型和季节
            land_types_all.append(land_type)
            seasons_all.append(season_part.strip())
